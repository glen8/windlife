    ...
    function insertSearch(id, catchRemote) {
        ...
        for (var i = 0, ci; ci = imgs[i++];) {
            if (ci.getAttribute("selected")) {
                ...
                img.src = url;
                img._src = url;
                if(ci.getAttribute("alt", 2)!=null){
                	img.alt=ci.getAttribute("alt", 2).replace(/(\s*$)/g, "");
                }
                imgObjs.push(img);
            }
        }
                ...
                if (id == "imgManager") {
                    ...
                    if (!list.children.length) {
                        ajax.request(editor.options.imageManagerUrl, {
                            timeout:100000,
                            action:"get",
                            image_path: editor.options.image_path,
                            YII_CSRF_TOKEN:editor.options.YII_CSRF_TOKEN,
                            onsuccess:function (xhr) {
                                ...
                                for (var k = 0, ci; ci = imageUrls[k++];) {                                	
                                    ...
                                    img.setAttribute(k < 35 ? "src" : "lazy_src", editor.options.imageManagerPath + imageinfo[0].replace(/\s+|\s+/ig, ""));
                                    img.setAttribute('alt',imageinfo[1].replace(/\s+|\s+/ig, ""));
                                    img.setAttribute("_src", editor.options.imageManagerPath + imageinfo[0].replace(/\s+|\s+/ig, ""));
