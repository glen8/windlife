...
class Advert extends BaseModel
{
	...
	public function rules()
	{
		...
		return array(
			array('position_key, ad_key', 'required'),
			array('width, height, num', 'numerical', 'integerOnly'=>true),
			array('position_key, ad_key', 'length', 'max'=>20),
			array('description', 'length', 'max'=>255),
			array('style', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, position_key, ad_key, description, width, height, num, style', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'items'=>array(self::HAS_MANY, 'AdvertItem', 'ad_id', 'order'=>'listorder ASC'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('position_key',$this->position_key,true);
