...
class EAdvancedArBehavior extends CActiveRecordBehavior {
	...
	public function fixBELONGS_TO() {
		...
		foreach ( $owner->relations () as $key => $relation ) {
			/*
			 * $key -> relation name $relation[1] -> related table $relation[2] -> foreignkey field
			 */
	...
	/**
	 * At first, this function cycles through each MANY_MANY Relation.
	 * Then
	...
	private function loadOldRelationsFromDB($model) {
		...
		$reader = $this->owner->dbConnection->createCommand ( sprintf ( 'SELECT %s FROM %s WHERE %s = %d', $secondRelation, $this->getManyManyTable ( $model ), $this->getManyManyFirstRelationName ( $model ), $this->owner->primaryKey ) )->query ();
	 ...
	 *
	 * @param string $model
	 *        	( e.g: tbl_project_user_assignment(user_id, project_id) )
