...
class Tree {
	...
	*/
	 ...
	 * @return init
	 */
	public function get_level($myid){
		if($this->arr[$myid]['parentid']==0)return false;
		self::$level++;
		$this->get_level($this->arr[$myid]['parentid']);
		return self::$level;
	}
	...
	*/
	 ...
	 * 同上一方法类似,返回数组
	 */
	public function get_tree_array($myid, $adds = ''){
		$number=1;
		$child = $this->get_child($myid);
		if(is_array($child)){
			$total = count($child);
			foreach($child as $id=>$a){
				...
				$spacer = $adds ? $adds.$j : '';	
				$this->ret_array[$id]=$spacer.$a['name'];
				$this->get_tree_array($id, $adds.$k.'&nbsp;');
				$number++;
			}
		}
	 ...
	 * 同上一方法类似,返回数组
	 */
	public function get_tree_carray($myid, $adds = ''){
		$number=1;
		$child = $this->get_child($myid);
		if(is_array($child)&&count($child)>0){
			...
			foreach($child as $id=>$a){
				...
				$spacer = $adds ? $adds.$j : '';
				$a['colname']=$spacer.$a['colname'];
				$this->ret_array[$id]=$a;
				$this->get_tree_carray($id, $adds.$k.'&nbsp;');
				$number++;
			}
		}
		return $this->ret_array;
	...
	*/
	...
	public function delFirstAarray($delarr,$is_flag=0){
		$newarr=array();
		if(is_array($delarr)){
			array_shift($delarr);
			$newarr=$delarr;
			if(count($delarr)==1&&$is_flag==1){
				foreach ($delarr as $v){
					$newarr=$v;
				}
			}
