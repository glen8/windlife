(function($){
	var bigColorPicker = new function(){
		...
		this.init = function(){
			$("body").append('<div id="bigpicker" class="bigpicker"><ul class="bigpicker-bgview-text" ><li><div id="bigBgshowDiv"></div></li><li><input id="bigHexColorText" size="7" maxlength="7" value="#000000" /></li><li><a class="bold"></a><a class="em"></a></li><li class="last"><a rel="bigpicker_clear">清除</a>/<a rel="bigpicker_close">关闭</a></li></ul><div id="bigSections" class="bigpicker-sections-color"></div><div id="bigLayout" class="biglayout" ></div></div>');
			...
			$('a[rel=bigpicker_close]').click(function(){
				$("#bigpicker").hide();
			});
			
			$('a.bold').click(function(){
			    invokeCallBack('bold','font-weight');
			});
			
			$('a.em').click(function(){
			    invokeCallBack('oblique','font-style');
			});
...
function style_group(str,style,value){
	if(str==''){
		return style+':'+value;
	}
	else{
		...
		for(i=0;i<str_array.length;i++){
			if(str_array[i].indexOf(style)>=0){
				str_array[i]=style+':'+value;
				flag=true;
				break;
			}
		}
		if(!flag){
			str_array[str_array.length]=style+':'+value;
		}
